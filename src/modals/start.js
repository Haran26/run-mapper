    ...
    if (isActive) {
      (async () => {
        ...
        if (lat1 != null && lon1 != null) {
          ...
          if (runPath.length > 15) {
            let c = runPath.length - 1;
            let currentDistanceTraveled =
              distanceInKmBetweenEarthCoordinates(runPath[c - 15].latitude, runPath[c - 15].longitude, runPath[c].latitude, runPath[c].longitude) *
              1000;
            setCurrentSpeed(calculateAvgSpeed(currentDistanceTraveled, 15));
          }
        }
      })();
      //format time
